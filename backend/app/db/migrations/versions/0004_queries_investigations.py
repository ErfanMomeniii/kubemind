...
def upgrade() -> None:
    # Create investigations first without the query_id FK (circular dep with queries).
    op.create_table(
        "investigations",
    ...
    # Now add the investigations.query_id FK (queries table exists).
    op.create_foreign_key(
        "fk_investigations_query_id_queries",
        "investigations",
        "queries",
        ["query_id"],
        ["id"],
...
def downgrade() -> None:
    op.drop_constraint("fk_investigations_query_id_queries", "investigations", type_="foreignkey")
